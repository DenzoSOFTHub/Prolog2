...
public class Lexer {
    ...
    private Token readToken() throws IOException {
        ...
        if (Character.isLetter(currentChar) || currentChar == '_' || currentChar == '\'') {
            ...
            if (value.equals("is")) {
                return new Token(Token.Type.IS, value, startLine, startColumn);
            }
            
            // Check if it's a variable (starts with uppercase) or atom
            if (Character.isUpperCase(value.charAt(0)) || value.charAt(0) == '_') {
        ...
        switch (ch) {
            ...
            case '*': 
                if (currentChar == '*') {
                    currentChar = reader.read();
                    column++;
                    return new Token(Token.Type.CARET, "**", startLine, startColumn);
                }
            ...
            case '=': 
                if (currentChar == '<') {
                    ...
                    currentChar = reader.read();
                    column++;
                    return new Token(Token.Type.EQEQ, "==", startLine, startColumn);
                } else if (currentChar == '.') {
                    currentChar = reader.read();
                    column++;
                    // This would be =. for univ operator, but we'll simplify
                } else if (currentChar == '\\') {
                    currentChar = reader.read();
                    column++;
                    return new Token(Token.Type.NEQ, "=\\=", startLine, startColumn);
                } else if (currentChar == ':') {
                    currentChar = reader.read();
                    column++;
            ...
            case '!': return new Token(Token.Type.CUT, "!", startLine, startColumn);
            default: 
